...
from SchNet4AIM.nn.initializers import zeros_initializer
...
class DummyAggregate(nn.Module):                                           
    """
    Defines a pooling layer that maps the input vector into an output array rather
    than accumulating the individual elements into a molecular scalar.
    """
    ...
    def forward(self, input, mask=None):
        r"""Passes input tensor into output tensor
        """
        # output as simply the input
        output = input
        return output                                        
